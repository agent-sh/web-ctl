...
describe('isAutoGeneratedId', () => {
  ...
  it('detects hex strings at the 6-char boundary', () => {
    assert.equal(isAutoGeneratedId('abcdef'), true);
    assert.equal(isAutoGeneratedId('a1b2c3d4e5'), true);
    assert.equal(isAutoGeneratedId('ABCDEF'), true);
    // 5 hex chars is below threshold
    assert.equal(isAutoGeneratedId('abcde'), false);
  });

  it('detects numeric-only IDs', () => {
    assert.equal(isAutoGeneratedId('12345'), true);
    assert.equal(isAutoGeneratedId('0'), true);
  });

  it('detects letter-prefixed hex IDs (case-insensitive)', () => {
  ...
  it('detects framework-prefixed IDs', () => {
    ...
    assert.equal(isAutoGeneratedId('ng-c123'), true);
    assert.equal(isAutoGeneratedId('rc-tabs-0'), true);
    assert.equal(isAutoGeneratedId('__next'), true);
  });

  it('detects colon and dot-containing IDs', () => {
    assert.equal(isAutoGeneratedId(':r0:'), true);
    assert.equal(isAutoGeneratedId('my.component.id'), true);
  });
  ...
  it('keeps human-readable IDs', () => {
    ...
    assert.equal(isAutoGeneratedId('header-123'), false);
    assert.equal(isAutoGeneratedId('step-2'), false);
  });
});
...
describe('buildSelector', () => {
  ...
  it('stops at human ID deep in ancestor chain', () => {
    // ancestors ordered child-first: [immediate parent, grandparent, ...]
    const ancestors = [
      { tagName: 'DIV', id: '', className: 'inner' },
      { tagName: 'SECTION', id: 'my-section', className: 'content' },
      { tagName: 'DIV', id: 'a1b2c3d4', className: 'outer' },
    ];
    // Should stop at my-section, not traverse further
    assert.equal(buildSelector(ancestors, 'span'), '#my-section > div.inner > span');
  });

  it('handles deep ancestor chains (5+ levels)', () => {
    const ancestors = [
      { tagName: 'LI', id: '', className: 'item' },
      { tagName: 'UL', id: '', className: 'list' },
      { tagName: 'DIV', id: '', className: 'wrapper' },
      { tagName: 'SECTION', id: '', className: 'content' },
      { tagName: 'MAIN', id: '', className: '' },
    ];
    assert.equal(
      buildSelector(ancestors, 'a'),
      'main > section.content > div.wrapper > ul.list > li.item > a'
    );
  });
  ...
  it('never produces double > > in output', () => {
    ...
    assert.equal(result, 'body > main > div');
  });
});
